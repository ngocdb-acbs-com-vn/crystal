...
interface VirtuosoRichOutputViewWithSearchProps {
  sessionId: string;
...
export const VirtuosoRichOutputViewWithSearch = React.forwardRef<{ scrollToPrompt: (promptIndex: number) => void }, VirtuosoRichOutputViewWithSearchProps>(
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      ...
      // Smart update: only append new messages if we already have messages
  ...
  const performSearch = useCallback(async () => {
    ...
    // Search through filtered messages (considering showSessionInit setting)
    ...
    for (let i = 0; i < searchLimit; i++) {
      ...
      const messageResults: SearchResult['matches'] = [];
      ...
      message.segments.forEach((segment, segmentIndex) => {
        if (segment.type === 'text') {
          ...
          while (matchIndex !== -1 && messageResults.length < 5) { // Limit to 5 matches per message
            messageResults.push({
              segmentIndex,
        ...
        // Could also search in tool results if needed
      });
  ...
  // Copy EXACT renderMessage function from RichOutputView - with search highlighting
  ...
  }, [filteredMessages, collapsedMessages, expandedTools, settings, searchQuery, searchResults, currentSearchIndex]);
  ...
  return (
    <div className="h-full flex flex-col bg-bg-primary relative">
      {/* Search Bar */}
      {isSearchOpen && (
        <div className="absolute top-4 right-4 z-20 flex items-center gap-2 bg-surface-secondary border border-border-primary rounded-lg p-2 shadow-lg">
      ...
      {!isSearchOpen && (
        <button
          ...
          className="absolute top-4 right-4 z-20 p-2 bg-surface-secondary hover:bg-surface-hover border border-border-primary rounded-lg transition-colors"
      ...
      <div className="flex-1 relative">
          ...
          itemContent={(index) => {
            ...
            return (
              <div className={`mx-auto ${settings.compactMode ? 'max-w-6xl' : 'max-w-5xl'} px-4`}>
                ...
                {index === filteredMessages.length - 1 && <div className="h-4" />}
              </div>
            );
          }}
          followOutput={'smooth'}
        ...
        {messageCountDisplay && (
          <div className="absolute bottom-1 right-2 text-[10px] text-text-tertiary/50 pointer-events-none">
            {messageCountDisplay}
          </div>
        )}
...
VirtuosoRichOutputViewWithSearch.displayName = 'VirtuosoRichOutputViewWithSearch';